...
- **Geração dos ativos:**
  ...
  - 8 flashcards (frente/verso, com flip 3D)
  - Quiz com 5 questões objetivas, gabarito, explicação e score final
...
```
Config             — endpoints, modelo, limites
DOMUtil            — sanitização + renderer de Markdown seguro (sem innerHTML)
ApiKeyStore        — persistência da chave em sessionStorage
...
└── js/
    ...
    ├── api-key-store.js          sessionStorage
...
3. (Opcional) Abra a **Bússola** (botão flutuante) e pergunte *"O que é o Neo Copilot?"* — ela responde sem precisar de PDF, só com a base do produto
4. Solte um ou mais PDFs (o app vem com `case_neo_copilot.pdf`) na dropzone
5. Assista o pipeline executar e navegue pelas 5 abas geradas
6. Volte na **Bússola** e pergunte sobre o conteúdo dos PDFs — ela cita a fonte específica em cada resposta
...
**Este é um protótipo de demonstração.** A chave da API é digitada pelo usuário no modal e armazenada em `sessionStorage` (limpa ao fechar a aba). É enviada direto do navegador para `generativelanguage.googleapis.com`.
...
O tier gratuito do Gemini limita a 10 req/min. O app não implementa retry com backoff — se você bater o limite, aparece um toast de erro e é só aguardar 1 minuto.
